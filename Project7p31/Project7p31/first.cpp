...
main(int argc, char *argv[])
{
	...
	if (fp1 == NULL)
	{
		fprintf(stderr, "Cannont open file 'signal.txt\n");
		exit(1);
	}
	int Size;
	fscanf(fp1, "%d", &Size);
	...
	for (int i = 0; i < SIZE; i++)
	{
		hArray[i] = hSums[i] = (float)sqrt((double)i);
	}
	...
	cl_mem dSums = clCreateBuffer(context, CL_MEM_READ_ONLY, 1 * Size * sizeof(cl_float), NULL, &status);
	if (status != CL_SUCCESS)
	...
	for (int i = 0; i < Size; i++)
	{
		float expected = hArray[i] * hSums[i];
	...
	fprintf(stderr, "%8d\t%4d\t%10d\t%10.3lf GigaMultsPerSecond\n",
		NMB, LOCAL_SIZE, NUM_WORK_GROUPS, (double)(SIZE*SIZE) / (time1 - time0) / 1000000.);
