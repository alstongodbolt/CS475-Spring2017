...
main( int argc, char *argv[ ] )
{
	...
	size_t numWorkGroup = NUM_ELEMENTS / LOCAL_SIZE;
	...
	float *hC = new float[ numWorkGroup ];
	size_t abSize = NUM_ELEMENTS * sizeof(float);
	size_t cSize = numWorkGroup * sizeof(float);
	...
	status = clEnqueueReadBuffer( cmdQueue, dC, CL_TRUE, 0, numWorkGroup*sizeof(float), hC, 0, NULL, NULL );
	if( status != CL_SUCCESS )
			fprintf( stderr, "clEnqueueReadBuffer failed\n" );
	//PrintCLError(status, "clEnqueueReadBuffer: ")
	Wait(cmdQueue);
	// did it work?
	float sum = 0;
	for( int i = 0; i < numWorkGroup; i++ )
	{
		sum += hC[i];
